const items = [
  ...
  function logItemNames() { items.forEach((element)=> console.log(element))}
   ...
   */
  function findItemById(id) { items.find(x=>x.id==='1')
  return id
  ...
  function calculateTotalInventory() {
    var total= items.reduce((accumulator,initalValue)=>accumulator+initalValue)
    return total
   ...
   */
  function calculateAllInventoryPrice() {const initalValue=0;
    const number= items.reduce((accumulator,currentValue)=> accumulator+currentValue,initalValue);
    return number
  ...
  function getItemPriceByName(name) {
    for (let i=0; i<items.length;i++){
        ...
        else return 'No item found with that price'}
    }
  ...
  function filterItemsByCategoryId(categoryId) {
    const result= items.filter((items)=> items.length>5)
    return result
